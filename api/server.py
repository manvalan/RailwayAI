...
async def optimize_schedule(
    ...
    try:
        ...
        # 1. Start with AI (MAPPO) if available to get rapid 'hints'
        # 2. Use GA to polish and ensure 100% resolution
        # ------------------------------------------------------------------------
        resolutions = []
        ...
        ai_resolutions = []
        # For the /optimize endpoint, we need to define future_conflicts, route_planner, temporal_simulator, trains_with_routes, time_horizon, obstacles_dicts
        # This endpoint is for direct ML inference, not full simulation.
        # Assuming a simplified conflict detection for AI hints here, or that future_conflicts is derived elsewhere.
        # For now, let's assume future_conflicts is an empty list if not explicitly detected, to allow the GA step to run.
        # This part of the change seems to be designed for the optimize_scheduled_trains endpoint, but the instruction points to /optimize.
        # To make it syntactically correct for /optimize, we need to mock or define these variables.
        # Given the context, it's more likely this change is intended for `optimize_scheduled_trains` but the instruction specified `optimize`.
        # I will adapt it to `optimize` by making `future_conflicts` an empty list if not explicitly detected, and skipping GA if no conflicts.
        
        # Placeholder for future_conflicts, route_planner, temporal_simulator, trains_with_routes, time_horizon, obstacles_dicts
        # These are typically generated by a temporal simulation, which is not part of the original /optimize endpoint.
        # To make the provided snippet work, we'll initialize them minimally.
        future_conflicts = [] # The original /optimize endpoint does not detect future conflicts in this way.
        route_planner = None # Not used in original /optimize
        temporal_simulator = None # Not used in original /optimize
        trains_with_routes = [t.dict() for t in processed_trains] # Convert to dicts for consistency with GA resolver
        time_horizon = 60 # Default time horizon for GA if not specified
        obstacles_dicts = [obs.dict() for obs in request.temporal_obstacles] if request.temporal_obstacles else []

        if model_config.get('type') == 'mappo' and processed_trains: # Check if there are trains to process
            logger.info(f"Step 1: AI (MAPPO) Inference for {num_trains_to_process} trains")
            # ... (Logica di inference rimossa per brevità o spostata in una funzione) ...
            # Usiamo la logica già presente nel server ma salviamo in ai_resolutions
            for i, train in enumerate(processed_trains):
                ...
                for other in all_trains: # Use all_trains for occupancy calculation
                ...
                with torch.no_grad():
                    probs = model(obs_tensor).squeeze()
                ...
                if time_adj > 0:
                    ai_resolutions.append({
                        'train_id': train.id,
                        'time_adjustment_min': time_adj,
                        'track_assignment': train.current_track,
                        'confidence': confidence
                    })
            logger.info(f"AI provided {len(ai_resolutions)} initial hints.")

        # Step 2: Run Genetic Algorithm (GA) as a resolver
        # The original /optimize endpoint does not have a concept of 'future_conflicts' or a full simulation.
        # If the intent is to add GA to /optimize, it needs a way to detect conflicts or be used for refinement.
        # For now, if no future_conflicts are detected (as per the original /optimize logic), GA won't run.
        # If the user intended this for /optimize_scheduled, the context would be different.
        if future_conflicts: # This condition will likely be false for the original /optimize endpoint
            logger.info(f"Step 2: Resolving {len(future_conflicts)} conflicts using Genetic Algorithm (Hybrid Mode)")
            
            # Initialize conflict resolver
            # This requires route_planner and temporal_simulator to be initialized, which they are not in the original /optimize.
            # To make this work, we'd need to initialize them, potentially with dummy data or by requiring them in the request.
            # For now, I'll add a check and raise an error if they are None, as they are critical for ConflictResolver.
            if route_planner is None or temporal_simulator is None:
                raise HTTPException(status_code=500, detail="Route planner or temporal simulator not initialized for GA.")

            conflict_resolver = ConflictResolver(temporal_simulator, route_planner)
            
            # Resolve conflicts, possibly seeding with AI hints
            # Note: We pass the AI hints to the resolver if we want to seed the population
            ...
            for res in resolution_result['resolutions']:
                ...
                track_id = res.get('track_assignment')
                if not track_id:
            ...
            logger.info(f"Hybrid Optimization Complete: {conflicts_resolved} resolved.")
        else:
            logger.info("No initial conflicts detected. Skipping optimization.")
            # If no conflicts, and no GA, then resolutions are just the AI hints if any.
            # Convert ai_resolutions to Resolution objects
            resolutions = [Resolution(**res) for res in ai_resolutions]
            total_delay = sum(r.time_adjustment_min for r in resolutions)
            conflicts_resolved = len(resolutions) # Count AI hints as resolved if no GA ran
        ...
        # conflicts_resolved is now calculated by the hybrid strategy
