...
async def optimize_schedule(
    ...
    try:
        ...
        # ------------------------------------------------------------------------
        # PURE AI OPTIMIZATION (MAPPO)
        # 1. AI provides rapid 'hints' based on current learning
        # 2. Results are returned to the App which handles final validation/GA
        # ------------------------------------------------------------------------
        resolutions = []
        ...
        if model_config.get('type') == 'mappo' and processed_trains:
            ...
            for i, train in enumerate(processed_trains):
                ...
                # We return the suggestion if AI is at least 40% confident or wants a change
                time_adj = 0.0
            ...
            conflicts_resolved = len([r for r in resolutions if r.time_adjustment_min > 0])
            logger.info(f"AI provided {len(resolutions)} suggestions.")
        else:
            logger.info("No trains or no MAPPO model loaded. Skipping AI optimization.")
